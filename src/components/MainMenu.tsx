    ...
    <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-black via-slate-950 to-cyan-950/20 flex justify-center items-center overflow-hidden">
      ...
      <div className={`relative z-10 text-center space-y-6 p-8 transition-all duration-1000 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="relative mb-8">
          <h1 className={`text-5xl md:text-7xl font-bold text-cyan-50 tracking-wide filter drop-shadow-[0_0_20px_rgba(0,150,200,0.8)] ${flicker ? 'opacity-70' : ''}`}
            style={{ 
          ...
          <h1 className={`text-6xl md:text-8xl font-bold text-cyan-500 tracking-wider mt-2 animate-pulse ${flicker ? 'opacity-50' : ''}`}
            style={{ 
              textShadow: '3px 3px 6px rgba(0,0,0,0.9), 0 0 60px rgba(0,139,139,0.8)',
              fontFamily: 'monospace'
            }}>
            STAR ABYSS
          </h1>
          ...
          <div className="absolute top-0 left-0 w-full opacity-30 animate-glitch-horror" style={{ color: '#0088AA' }}>
            ...
            <h1 className="text-6xl md:text-8xl font-bold tracking-wider mt-2" style={{ fontFamily: 'monospace' }}>
              STAR ABYSS
            </h1>
          </div>
        </div>
        ...
        <p className="text-cyan-400/60 text-xs md:text-sm font-mono mt-4 animate-pulse italic">
          &gt; No rescue available. Survival not guaranteed.
        </p>

      </div>

      {/* Corrupted Corner Decorations */}
      <div className="absolute top-0 left-0 w-24 h-24 border-l-2 border-t-2 border-cyan-800/40" 
        style={{ clipPath: 'polygon(0 0, 100% 0, 100% 15%, 95% 20%, 0 20%, 0 100%)' }} />
      <div className="absolute top-0 right-0 w-24 h-24 border-r-2 border-t-2 border-cyan-800/40"
        style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 100% 20%, 5% 20%, 0 15%)' }} />
      <div className="absolute bottom-0 left-0 w-24 h-24 border-l-2 border-b-2 border-cyan-800/40"
        style={{ clipPath: 'polygon(0 0, 0 100%, 100% 100%, 100% 85%, 5% 80%, 0 80%)' }} />
      <div className="absolute bottom-0 right-0 w-24 h-24 border-r-2 border-b-2 border-cyan-800/40"
